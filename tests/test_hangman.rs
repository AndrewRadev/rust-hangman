...
fn test_game_error() {
    ...
    assert_match!(Some(GameError::InvalidSolution(_)), Game::new(" foo", 10).err());
...
fn test_command_parsing_partial_words() {
    assert_match!(Ok(Command::Quit), "qwit".parse::<Command>());
    ...
    assert_match!(Ok(Command::TryLetter('_')), "t l _".parse::<Command>());
    assert_match!(Ok(Command::TryLetter('c')), "too late c".parse::<Command>());

    assert_match!(Err(GameError::ParseError(_)), "t t t".parse::<Command>());

    assert_match!(Ok(Command::TryWord(_)), "t w c".parse::<Command>());
    assert_match!(Ok(Command::TryWord(_)), "t w critical".parse::<Command>());
    assert_match!(Ok(Command::TryWord(_)), "txzf www callback".parse::<Command>());
